...
만약 자신이 운영하고 있는 이더리움 노드가 있다면, 해당 노드의 접속 주소를 위의 방법으로 확보한 `Infura` 주소 대신 사용할 수 있다.

### `ChainID` 확인

토카막 네트워크는 여러 오퍼레이터가 각자 자신의 자식체인을 운영하는 구조를 가지고 있다. 이때, 각기 고유한 루트체인 컨트렉트 주소를 가지게 되나, 루트체인 컨트랙트 배포시 사용되는 `ChainID` 는 중복 될 수 있다.

이는 [`Replay Attack`]() 이 가능한 요소이다. 예를들어 오퍼레이터A 와 오퍼레이터B 모두 동일한 `ChainID`를 사용하고 있다면, 오퍼레이터A 자식체인에서 처리된 트랜잭션을 누구나 가져와 오퍼레이터B의 자식체인에서도 사용 할 수 있다.

이 `Replay Attack`을 방지하기 위해서는 오퍼레이터 서로가 겹치지 않은 고유의 `ChainID`를 사용해야 한다. 토카막 네트워크에서는 이를 시뇨리지 메니저 에서 관리하게 되며, 동일한 `ChainID`를 사용할 수 없도록 한다.

먼저, 오퍼레이터가 루트체인 컨트랙트를 배포하기 전에, 반드시 아래 링크에서 자신이 사용하고자 하는 `ChainID` 가 이미 등록되어 있는지 확인해야 한다.
...
<!- TODO: check which part in, Before rootchain contract deploy or After some part.. ->

### 루트체인 컨트랙트 배포

먼저, 아래 명령어를 통해 오퍼레이터 계정을 생성한다.

오퍼레이터 노드에서 사용할 카파일 암호를 `Password` 에 입력한다.
...
Password:
Repeat password:
Address: {57ab89f4eabdffce316809d790d5c93a49908510}
```
...
DESCRIPTION="This is test operator"
```
...
루트체인이 메니저 컨트랙트 등록이 정상적으로 이루어 졌다면 아래와 같이 10,000 TON 의 잔고를 확인할 수 있다.
