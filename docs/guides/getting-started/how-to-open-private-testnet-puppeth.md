...
sidebar_label: Puppeth 사용하여 설정하기
---

`Puppeth`란 이더리움 노드 배포를 쉽게 할 수 있는 유틸리티 프로그램으로서 내부적으로 `Docker`를 사용한다.<br>
`plasma-evm`의 `Puppeth`는 토카막 노드를 쉽게 구성 할 수 있도록 추가된 기능이 탑재 되어 있다.

`Puppeth`를 사용하여 자식 체인을 설정하려면 직접 설치와 마찬가지로 사용가능한 루트체인이 필요하다.<br> 루트체인이 실행중이지 않고 있다면 [루트체인 설정하기](how-to-open-private-testnet-rootchain#%EB%B6%80%EB%AA%A8-%EC%B2%B4%EC%9D%B8-%EC%84%A4%EC%A0%95%ED%95%98%EA%B8%B0)를 먼저 진행한다.

이 가이드에서는 `Puppeth`를 사용하여 원격 머신으로서 자신의 로컬 환경을 사용 한다.

`Puppeh`에서 사용되는 용어를 정리하면 아래와 같다.
...
> 루트체인으로 `ganache` 테스트체인을 사용하는 경우, `ganache` 실행시 생성된 계정을 오퍼레이터(Operator)와 첼린저(Challenger)로 사용하여야 한다.
...
`Puppeth`를 실행하기 이전에 자식체인과 연결될 루트체인 컨트렉트를 배포하고 해당 정보를 담은 `genesis` 파일을 생성한다.
<br>[프라이빗 테스트넷 직접 설정하기 - 1. 루트체인 컨트렉트 배포하기](how-to-open-private-testnet-manually#1-%EB%A3%A8%ED%8A%B8%EC%B2%B4%EC%9D%B8-%EC%BB%A8%ED%8A%B8%EB%A0%89%ED%8A%B8-%EB%B0%B0%ED%8F%AC%ED%95%98%EA%B8%B0) 와 동일하게 `deploy` 커맨드를 사용하여 루트체인 contract을 배포한후 `genesis.json` 파일을 생성하는 스크립트를 작성한다.
...
``` 
생성된 `genesis.json` 파일은 동일하게 `plasma-evm` 폴더 내에 위치한다.


### 3. `Puppeth` 실행하기

puppeth를 실행하기 위해서는 `build/bin/puppeth` 명령어를 입력하여 실행한다. ( go1.13 버전을 사용하는 것을 추천한다)
...
글의 맨처음 이야기 하였지만, `Puppeth`를 사용하기 위해서는 원격 머신에 Docker가 설치 되어 있어야 한다. 로컬 환경에 `Docker`가 설치 되어 있음을 전제 한다.
호스트 머신 환경별 `Docker` 설치에 관해서는 [외부 문서](https://docs.docker.com/install/#supported-platforms)를 참고한다.

### 1. `genesis` 파일 불러오기

`Puppeth`를 통해서 노드들을 설정하기 위해서는 `genesis.json`파일이 필요하다.
...
자신의 MacOS에 설치하는 경우라고 한다면 우선 원격 컴퓨터가 MacOS에 접근하도록 허용해 주어야 한다. 
...
`Ethstats` 정보를 가지고 있어야 사용자 노드 컨테이너 생성이 가능하다. 따라서, 3번 과정을 먼저 수행해 주어야 한다.
...
루트체인 JSONRPC는 로컬 머신 에서 작동하고 있는 루트체인 웹소켓(websocket) 접속 주소를 적어준다.(`ws://127.0.0.1:8546`)
...
사용자가 노드에 접근하여 트랜젝션 보내려면 `HTTP JSONRPC endpoint` 를 `yes` 선택하여 반드시 열어주어야 한다.
...
## 오퍼레이터 노드 설정하기
자식체인의 블록을 생성하는 오퍼레이터 노드를 구성한다. `Puppeth`를 사용하여 오퍼레이터 노드 설정하기 이전에 `Ethstats` 와 사용자 노드(=`bootnode`) 가 미리 구성되어 있어야 한다.
...
사용자 노드와 동일한 루트체인 `JSONRPC` 주소를 입력해준다.
...
오퍼레이터와 첼린저 계정은 루트체인에 잔고(Balance)가 있는 계정을 사용해야 한다.
...
원하는 경우 `What gas limit should full blocks target?` 의 숫자를 높여 오퍼레이터 노드가 생성하는 블록에서 더 많은 트렌젝션을 처리 할 수 있도록 한다.
...
`Puppeth` 를 통해 로컬 환경을 원격 머신처럼 사용하여 `Ethstats`, `Bootnode`, `Nginx` 그리고 `Sealnode` 컨테이너를 생성하였다.

은 아래와 같이 `Puppeth`에서 조회 된다.
